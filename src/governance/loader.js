...
function resolveDirectory(entry, root) { try { const resolved = fs.realpathSync(entry); return fs.statSync(resolved).isDirectory() && isContained(root, resolved) ? { ok: true, path: resolved } : { ok: false, code: "invalid-governance-root" }; } catch (cause) { if (cause.code === "ENOENT") { try { fs.lstatSync(entry); return { ok: false, code: "invalid-governance-root" }; } catch (entryCause) { if (entryCause.code === "ENOENT") return { ok: false, code: "missing-manifest" }; throw internalError(entryCause); } } if (cause.code === "ELOOP") return { ok: false, code: "invalid-governance-root" }; throw internalError(cause); } }

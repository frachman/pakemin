...
- Milestone 8 Slice 1 remediation and Slice 2 actual-path resolver are accepted internally: load-time coverage is 36/36, resolution-time coverage is 2/2 through executable keyed fixtures, and normalized resolver input enforcement plus the full resolver acceptance matrix are complete. Git comparison and change collection, normalized change-set validation, rule evaluation, exception application to verdicts, evidence/report construction, `pakemin check`, and the deferred task-envelope contract remain out of scope.
